...
- [ ] Test function names are backtick-quoted and built from ` - `-separated segments — a path from the thing under test down to the behavior, each hierarchy level its own segment: `UiEvent - <Event> - <behavior>` (event handling), `UiState - <field> - <behavior>` (state derivation), `UI - <element/concern> - <behavior>` (Compose UI interaction tests). When a presenter has page-scoped `UiState`/`UiEvent` subtypes, the page is its own segment: `UiState - <Page> - <field> - <behavior>` / `UiEvent - <Page> - <Event> - <behavior>` — e.g. `` `UiEvent - CollectingContact - Save - dispatches CollectContact built from the field state` ``
